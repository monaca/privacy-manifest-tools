...
async function addPrivacyManifest(appPath, privacyManifestPath) {
    ...
    if (privacyManifestAlreadyExists) {
        ...
        try {
            fs.copyFile(privacyManifestPath, destPath);
        ...
        // PbxBuildFileセクション に PrivacyManifestを追加
        addToPbxBuildFileSection(proj, file);
        // PbxFileReferenceセクション に PrivacyMniafestを追加
        addToPbxFileReferenceSection(proj, file);
        // PbxGroupセクション の Appグループの children に PrivacyManifestを追加
        addToAppPbxGroup(proj, file);
        // PbxResourcesBuildPhaseセクションの Resourcesセクションの files に PrivacyManifestを追加
        addToPbxResourcesBuildPhase(proj, file);
    
        const text = proj.writeSync();
        try {
            fs.writeFile(pbxprojPath, text, 'utf8');
        } catch (err) {
